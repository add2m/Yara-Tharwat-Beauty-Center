...
if current_page == "booking":
    ...
    with st.expander("🔐 إدارة"):
        ...
        if pwd == ADMIN_PASSWORD:
            for i, rev in enumerate(all_revs):
                if "|" in rev:
                    if st.button(f"🗑️ حذف {rev.split('|')[1]}", key=f"del_{i}"):
                        handle_reviews("delete_one", i); st.rerun()
    if st.button("العودة للرئيسية"): st.query_params.clear(); st.rerun()
    ...
    # الصفحة الرئيسية (الوضع اللي أنت ظبطته في الصورة)
    st.markdown('<p style="color: #D4AF37; font-size: 14px; font-weight: bold; text-align: left; margin-top: -10px; padding-left: 10px;">👈 انقري على السهم هنا للمزيد</p>', unsafe_allow_html=True)
